...
from chiller_analysis import (
    ChillerPointInputs,
    calculate_chiller_point,
    build_chiller_table_from_rom,
    summarize_chiller_table,
)
from external_validation import (
    LBNL_FOULING_FACTORS,
    prepare_lbnl_chiller_case,
    add_relative_penalties,
    load_bin_summary,
    compare_rom_to_external,
    calibrate_beta,
    slope_agreement,
)
...
st.set_page_config(page_title="HVAC ROM-Degradation Suite v4", layout="wide")
...
with tabs[0]:
    ...
    fouling_cop_beta = st.number_input(
        "Fouling-to-COP sensitivity βf", min_value=0.0, value=float(st.session_state.get("fouling_cop_beta", 0.45)),
        step=0.01, format="%.4f", key="fouling_cop_beta",
        help="Legacy ROM default is 0.45. The External Published Validation tab can estimate a calibrated value from independent fouling data."
    )
    c1, c2, c3, c4 = st.columns(4)
...
cfg = HVACConfig(
    ...
    FOULING_COP_BETA=float(st.session_state.get("fouling_cop_beta", 0.45)),
...
with tabs[23]:
    st.subheader("Chiller Calculation — ROM physics and plant performance")
    render_tab_export_panel("Chiller Calculation")
    st.markdown(
        "This tab exposes the chiller calculation independently from the full building simulation. "
        "It uses the ROM cooling-COP degradation relation and reports PLR, effective COP, chiller power, "
        "whole-plant COP, kW/ton, specific electricity consumption, and the isolated fouling penalty."
    )

    st.markdown("### A. Single operating-point calculator")
    c1, c2, c3, c4 = st.columns(4)
    ch_q = c1.number_input("Cooling load (kW)", min_value=0.0, value=1200.0, step=50.0, key="ch_calc_q")
    ch_cap = c2.number_input("Nominal chiller capacity (kW)", min_value=1.0, value=float(getattr(cfg, "_Q_COOL_DES_KW", 2697.66) if hasattr(cfg, "_Q_COOL_DES_KW") else 2697.66), step=50.0, key="ch_calc_cap")
    ch_cop = c3.number_input("Nominal cooling COP", min_value=0.8, value=float(cfg.COP_COOL_NOM), step=0.1, key="ch_calc_cop")
    ch_tamb = c4.number_input("Ambient temperature (°C)", value=35.0, step=0.5, key="ch_calc_tamb")

    c1, c2, c3, c4 = st.columns(4)
    ch_years = c1.number_input("Years in service", min_value=0.0, value=0.0, step=0.5, key="ch_calc_years")
    ch_rf = c2.number_input("Fouling resistance Rf (m²·K/W)", min_value=0.0, value=0.0, step=1e-5, format="%.7f", key="ch_calc_rf")
    ch_rf_star = c3.number_input("Rf* (m²·K/W)", min_value=1e-8, value=float(cfg.RF_STAR), step=1e-5, format="%.7f", key="ch_calc_rfstar")
    ch_beta = c4.number_input("Fouling sensitivity βf", min_value=0.0, value=float(getattr(cfg, "FOULING_COP_BETA", 0.45)), step=0.01, key="ch_calc_beta")

    c1, c2, c3 = st.columns(3)
    ch_pump = c1.number_input("Pump power (kW)", min_value=0.0, value=50.0, step=5.0, key="ch_calc_pump")
    ch_tower = c2.number_input("Cooling-tower power (kW)", min_value=0.0, value=30.0, step=5.0, key="ch_calc_tower")
    ch_other = c3.number_input("Other plant power (kW)", min_value=0.0, value=0.0, step=5.0, key="ch_calc_other")
    ch_apply_plr = st.checkbox("Apply ROM part-load COP modifier", value=False, key="ch_calc_apply_plr")

    point = calculate_chiller_point(ChillerPointInputs(
        cooling_load_kw=ch_q,
        nominal_capacity_kw=ch_cap,
        nominal_cop=ch_cop,
        ambient_temp_c=ch_tamb,
        years_in_service=ch_years,
        cop_aging_rate_per_year=float(cfg.COP_AGING_RATE),
        fouling_resistance_m2k_w=ch_rf,
        asymptotic_fouling_resistance_m2k_w=ch_rf_star,
        fouling_cop_beta=ch_beta,
        apply_part_load_modifier=ch_apply_plr,
        plr_a=float(getattr(cfg, "PLR_A", 0.85)),
        plr_b=float(getattr(cfg, "PLR_B", 0.25)),
        plr_c=float(getattr(cfg, "PLR_C", -0.10)),
        plr_d=float(getattr(cfg, "PLR_D", 0.0)),
        plr_min_modifier=float(getattr(cfg, "PLR_MIN_MODIFIER", 0.55)),
        plr_max_modifier=float(getattr(cfg, "PLR_MAX_MODIFIER", 1.15)),
        pump_power_kw=ch_pump,
        cooling_tower_power_kw=ch_tower,
        other_plant_power_kw=ch_other,
    ))
    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("PLR", f"{point['PLR']:.3f}")
    m2.metric("Effective COP", f"{point['COP_effective']:.3f}")
    m3.metric("Chiller power", f"{point['chiller_power_kw']:.1f} kW")
    m4.metric("Plant COP", f"{point['plant_COP']:.3f}")
    m5.metric("Plant kW/ton", f"{point['kW_per_ton']:.3f}")
    st.dataframe(pd.DataFrame([point]).T.rename(columns={0: "value"}), use_container_width=True)

    st.markdown("### B. Chiller KPIs from a ROM result folder")
    ch_folder = Path(st.text_input("ROM result folder", st.session_state.get("last_result_dir", "scenario_run"), key="chiller_calc_folder"))
    ch_paths = find_result_paths(ch_folder)
    if ch_paths["daily"].exists():
        if st.button("Build chiller time-series analysis from ROM", type="primary", key="build_chiller_rom_analysis"):
            try:
                src = pd.read_csv(ch_paths["daily"])
                cht = build_chiller_table_from_rom(
                    src,
                    nominal_capacity_kw=ch_cap,
                    nominal_cop=ch_cop,
                    rf_star=ch_rf_star,
                ...
                chsum = summarize_chiller_table(cht)
                ch_folder.mkdir(parents=True, exist_ok=True)
                ch_ts_path = ch_folder / "chiller_calculation_timeseries.csv"
                ch_sum_path = ch_folder / "chiller_calculation_summary.csv"
                cht.to_csv(ch_ts_path, index=False)
                chsum.to_csv(ch_sum_path, index=False)
                st.session_state["chiller_analysis_df"] = cht
                st.session_state["chiller_analysis_summary_df"] = chsum
                st.success("Chiller analysis created from the official ROM result file.")
            except Exception as e:
                st.error(f"Chiller analysis failed: {e}")
        if isinstance(st.session_state.get("chiller_analysis_summary_df"), pd.DataFrame):
            st.dataframe(st.session_state["chiller_analysis_summary_df"], use_container_width=True)
        if isinstance(st.session_state.get("chiller_analysis_df"), pd.DataFrame):
            cht = st.session_state["chiller_analysis_df"]
            st.dataframe(cht.head(500), use_container_width=True)
            if PLOTLY_AVAILABLE and {"Q_cool_kw", "chiller_COP"}.issubset(cht.columns):
                fig = px.scatter(cht, x="Q_cool_kw", y="chiller_COP", color="R_f" if "R_f" in cht.columns else None, title="ROM chiller COP versus cooling load")
                st.plotly_chart(fig, use_container_width=True)
            download_file_button(ch_folder / "chiller_calculation_timeseries.csv", "Download chiller_calculation_timeseries.csv", key="dl_ch_ts")
            download_file_button(ch_folder / "chiller_calculation_summary.csv", "Download chiller_calculation_summary.csv", key="dl_ch_sum")
    else:
        st.info("Run a scenario first or point this tab to a folder containing the official daily/time-step ROM result file.")


with tabs[24]:
    st.subheader("External Published Validation — LBNL chiller-plant fouling benchmark")
    render_tab_export_panel("External Published Validation")
    st.markdown(
        "Use the independent LBNL/DOE chiller-plant fault dataset to validate the ROM's degradation sensitivity. "
        "The benchmark is normalized, so the comparison focuses on fouling-induced changes in COP and specific electricity consumption rather than absolute MWh across different buildings and climates."
    )
    st.info(
        "Upload the four LBNL files: ChillerPlant.csv, ChillerPlant_coolingtower_fouling_095.csv, "
        "ChillerPlant_coolingtower_fouling_080.csv, and ChillerPlant_coolingtower_fouling_065.csv. "
        "The app auto-detects the documented CWL_SEC_LOAD, CHL_POW, CT_POW and pump-power point families."
    )

    c1, c2, c3 = st.columns(3)
    ext_min_load = c1.number_input("Minimum cooling load retained (kW)", min_value=0.0, value=50.0, step=10.0, key="ext_min_load")
    ext_resample = c2.selectbox("External-data aggregation", ["1h", "3h", "6h", "1D"], index=0, key="ext_resample")
    ext_capacity = c3.number_input("Load-bin reference capacity (kW, 0 = auto)", min_value=0.0, value=0.0, step=100.0, key="ext_capacity")

    uploads = {}
    upload_specs = [
        ("Fault-free", 1.00, "ext_clean"),
        ("Fouling 95% HTC", 0.95, "ext_095"),
        ("Fouling 80% HTC", 0.80, "ext_080"),
        ("Fouling 65% HTC", 0.65, "ext_065"),
    ]
    cols_up = st.columns(4)
    for col, (label, htc, key) in zip(cols_up, upload_specs):
        uploads[label] = (htc, col.file_uploader(label, type=["csv", "gz"], key=key))

    validation_dir = Path(st.text_input("Validation output folder", "external_validation_run", key="external_validation_output_dir"))
    current_beta = st.number_input("ROM βf used for comparison", min_value=0.0, value=float(getattr(cfg, "FOULING_COP_BETA", 0.45)), step=0.01, key="external_validation_beta")
    target_label = st.selectbox(
        "Calibration target",
        ["Plant energy penalty", "Plant COP degradation"],
        key="external_validation_target_label",
    )
    target_col = "plant_energy_penalty_pct" if target_label == "Plant energy penalty" else "plant_COP_degradation_pct"

    if st.button("Run external fouling validation", type="primary", key="run_external_validation"):
        missing = [label for label, (_, up) in uploads.items() if up is None]
        if missing:
            st.error("Missing required LBNL files: " + ", ".join(missing))
        else:
            try:
                validation_dir.mkdir(parents=True, exist_ok=True)
                prepared = {}
                summaries = []
                for label, (htc, up) in uploads.items():
                    raw = pd.read_csv(up, low_memory=False)
                    prep, summ = prepare_lbnl_chiller_case(
                        raw, case_label=label, htc_factor=htc,
                        min_cooling_load_kw=ext_min_load, resample_rule=ext_resample,
                    )
                    prepared[label] = prep
                    summaries.append(summ)
                    prep.to_csv(validation_dir / f"prepared_{_slugify_name(label)}.csv", index=False)
                    del raw
                    gc.collect()

                ext_summary = add_relative_penalties(pd.DataFrame(summaries), clean_label="Fault-free")
                bins = load_bin_summary(prepared, capacity_kw=(ext_capacity if ext_capacity > 0 else None))
                comparison, metrics = compare_rom_to_external(ext_summary, beta=current_beta, target=target_col)
                best_beta, calib_curve = calibrate_beta(ext_summary, target=target_col, beta_min=0.0, beta_max=5.0, steps=5001)
                best_comparison, best_metrics = compare_rom_to_external(ext_summary, beta=best_beta, target=target_col)
                slopes = slope_agreement(ext_summary, beta=best_beta)

                ext_summary.to_csv(validation_dir / "lbnl_external_summary.csv", index=False)
                bins.to_csv(validation_dir / "lbnl_load_bin_summary.csv", index=False)
                comparison.to_csv(validation_dir / "rom_vs_lbnl_current_beta.csv", index=False)
                metrics.to_csv(validation_dir / "validation_metrics_current_beta.csv", index=False)
                best_comparison.to_csv(validation_dir / "rom_vs_lbnl_calibrated_beta.csv", index=False)
                best_metrics.to_csv(validation_dir / "validation_metrics_calibrated_beta.csv", index=False)
                calib_curve.to_csv(validation_dir / "beta_calibration_curve.csv", index=False)
                slopes.to_csv(validation_dir / "degradation_sensitivity_slope_agreement.csv", index=False)
                pd.DataFrame([{
                    "legacy_beta": current_beta,
                    "calibrated_beta": best_beta,
                    "calibration_target": target_col,
                    "note": "Calibrated beta is for the isolated external fouling benchmark and should be transferred to the full ROM only after engineering review of the fouling-state mapping.",
                }]).to_csv(validation_dir / "beta_calibration_recommendation.csv", index=False)

                st.session_state["ext_validation_summary_df"] = ext_summary
                st.session_state["ext_validation_bins_df"] = bins
                st.session_state["ext_validation_current_comparison_df"] = comparison
                st.session_state["ext_validation_current_metrics_df"] = metrics
                st.session_state["ext_validation_best_comparison_df"] = best_comparison
                st.session_state["ext_validation_best_metrics_df"] = best_metrics
                st.session_state["ext_validation_slopes_df"] = slopes
                st.session_state["ext_validation_best_beta"] = best_beta
                st.success(f"External validation completed. Best-fit βf = {best_beta:.4f} for target: {target_label}.")
            except Exception as e:
                st.error(f"External validation failed: {e}")

    if isinstance(st.session_state.get("ext_validation_summary_df"), pd.DataFrame):
        ext_summary = st.session_state["ext_validation_summary_df"]
        st.markdown("### 1. Published LBNL fouling response")
        st.dataframe(ext_summary, use_container_width=True)

        st.markdown("### 2. ROM vs external validation — current βf")
        st.dataframe(st.session_state.get("ext_validation_current_comparison_df", pd.DataFrame()), use_container_width=True)
        st.dataframe(st.session_state.get("ext_validation_current_metrics_df", pd.DataFrame()), use_container_width=True)

        st.markdown("### 3. Calibrated fouling sensitivity")
        best_beta = float(st.session_state.get("ext_validation_best_beta", current_beta))
        b1, b2 = st.columns(2)
        b1.metric("Legacy/current βf", f"{current_beta:.4f}")
        b2.metric("Best-fit external-validation βf", f"{best_beta:.4f}")
        st.dataframe(st.session_state.get("ext_validation_best_comparison_df", pd.DataFrame()), use_container_width=True)
        st.dataframe(st.session_state.get("ext_validation_best_metrics_df", pd.DataFrame()), use_container_width=True)
        st.dataframe(st.session_state.get("ext_validation_slopes_df", pd.DataFrame()), use_container_width=True)

        st.markdown("### 4. Load-matched validation")
        bins = st.session_state.get("ext_validation_bins_df", pd.DataFrame())
        st.dataframe(bins, use_container_width=True)

        if PLOTLY_AVAILABLE:
            comp = st.session_state.get("ext_validation_best_comparison_df", pd.DataFrame())
            if not comp.empty:
                y_external = target_col
                y_rom = "ROM_energy_penalty_pct" if target_col == "plant_energy_penalty_pct" else "ROM_COP_degradation_pct"
                long = comp[["htc_loss_pct", y_external, y_rom]].melt(id_vars=["htc_loss_pct"], var_name="series", value_name="penalty_pct")
                fig = px.line(long, x="htc_loss_pct", y="penalty_pct", color="series", markers=True, title="External LBNL response versus calibrated ROM fouling response")
                st.plotly_chart(fig, use_container_width=True)

        st.markdown("### 5. Validation KPI interpretation")
        st.markdown(
            "- **MAE/RMSE/NRMSE** quantify absolute and normalized disagreement in the degradation response.  \n"
            "- **MAPE** is calculated on faulted cases only; the zero-penalty clean case is excluded.  \n"
            "- **NMBE** indicates systematic over- or under-prediction of the published fouling penalty.  \n"
            "- **R²** is reported but should not be over-interpreted because only three independent fouling intensities are available.  \n"
            "- The **slope agreement** is the most physically useful cross-building indicator because it compares sensitivity to increasing fouling intensity."
        )

        for fname in [
            "lbnl_external_summary.csv", "lbnl_load_bin_summary.csv", "rom_vs_lbnl_current_beta.csv",
            "validation_metrics_current_beta.csv", "rom_vs_lbnl_calibrated_beta.csv",
            "validation_metrics_calibrated_beta.csv", "beta_calibration_curve.csv",
            "degradation_sensitivity_slope_agreement.csv", "beta_calibration_recommendation.csv",
        ]:
            download_file_button(validation_dir / fname, f"Download {fname}", key=f"dl_ext_{fname}")

    st.markdown("### Reproducibility note")
    st.caption(
        "This external-validation workflow isolates cooling-tower heat-exchanger fouling from filter clogging, generic COP aging, maintenance actions and S3 optimization. "
        "The LBNL 95%, 80% and 65% cases multiply the cooling-tower heat-transfer coefficient by those factors; they are not the same as the manuscript's Mild/Moderate/Severe/High multi-mechanism severity axis."
    )
