...
class HVACConfig:
    ...
    # Calibratable fouling-to-COP sensitivity used by the cooling COP model.
    # Default 0.45 preserves the legacy ROM behavior; the External Published Validation tab can estimate an alternative value.
...
def cop_cooling(cfg: HVACConfig, T_a: float, year_frac: float, rf: float) -> float:
    ...
    cop_foul = cop_aged / (1.0 + float(getattr(cfg, "FOULING_COP_BETA", 0.45)) * (rf / max(cfg.RF_STAR, 1e-12)))
...
def _append_dataframe_csv(df: pd.DataFrame, path: Path) -> None:
    """Append a dataframe to CSV while preserving a stable union schema.

    Earlier memory-safe builds could write a baseline frame with fewer columns than
    degraded scenarios, producing mixed-width CSV rows. This implementation expands
    the existing CSV header in a streaming pass whenever new columns appear, then
    reindexes every appended frame to the same union schema.
    """
    ...
    with path.open("r", newline="", encoding="utf-8") as f:
        reader = csv.reader(f)
        try:
            old_cols = next(reader)
        except StopIteration:
            old_cols = []

    union_cols = list(old_cols)
    for c in new_cols:
        if c not in union_cols:
            union_cols.append(c)

    # Expand existing rows without loading the full file into RAM.
    if union_cols != old_cols:
        tmp = path.with_suffix(path.suffix + ".schema_tmp")
        with path.open("r", newline="", encoding="utf-8") as src, tmp.open("w", newline="", encoding="utf-8") as dst:
            reader = csv.DictReader(src)
            writer = csv.DictWriter(dst, fieldnames=union_cols, extrasaction="ignore")
            writer.writeheader()
            for row in reader:
                writer.writerow({c: row.get(c, "") for c in union_cols})
        tmp.replace(path)

    aligned = df.reindex(columns=union_cols)
...
def train_surrogate_models(input_csv: str | Path, output_dir: str | Path, n_iter_search: int = 6, shap_sample: int = 1000, random_state: int = 42) -> Dict[str, str]:
    if not CATBOOST_AVAILABLE:
        raise ImportError("CatBoost is not installed. Install optional dependencies from requirements_ml_optional.txt")
